...
yum -y install epel-release vim tcpdump net-tools
...
yum install -y kubelet-1.21.14 kubectl-1.21.14 kubeadm-1.21.14
...
# kubeadm init --kubernetes-version=v1.21.14 --service-cidr=10.96.0.0/12 --apiserver-advertise-address=192.110.0.10 --ignore-preflight-errors=Swap --image-repository registry.aliyuncs.com/google_containers
